...
> 本文档定义仓库的目录结构、模块关系、统一规范与 SSOT 策略，是全仓结构性约束（v1.0 已按此落地）。任何新增内容不得与本文档冲突；确需调整时先修订本文档。
...
├── examples/                   # Phase 3：48 个独立可运行 Demo 工程（已交付）
├── projects/                   # Phase 4~6：三个企业级完整项目（已交付）
...
**模块挂载策略**：父 POM `<modules>` 挂载 `common` 与 `starter`；`examples/*` 与 `projects/*` 均为**独立 Spring Boot 应用**，以 `<parent>` 指向本父 POM 继承版本管理（不强制挂入父 modules；各自 `mvn -f …` / `spring-boot:run`）。
...
| Demo README 模板 | `examples/README.md` §3 | 48 个 Demo 统一 |
