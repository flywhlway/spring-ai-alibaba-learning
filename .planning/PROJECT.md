...
<!-- Shipped and confirmed valuable. Phase 1–2 brownfield delivered. -->
...
- [ ] **REQ-phase-3-demos** — Phase 3：48 个独立 Demo（examples/），编号/端口以 examples/README.md 为 SSOT
...
- **当前阶段：** Phase 3（48 个独立 Demo）。Phase 1（脚手架）与 Phase 2（教程 + starter + QA）已完成。
- **已交付：** `saa-learning-common`、`saa-learning-starter`、docker profiles（core/vector/mq/search/cloud）、scripts、docs/00-overview/01–04、docs/tutorial/01–22。
- **父 POM `<modules>`：** 当前仅 common + starter；examples/* 与 projects/* 为独立应用，按阶段挂载。
- **已知欠账：** starter 尚未真机 `mvn compile`（沙箱限制）→ 落地首检须 `mvn -pl common,starter -am clean install` 与 starter 单测。
