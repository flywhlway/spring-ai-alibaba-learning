...
/**
 * Spring Security 安全配置：JWT 资源服务器（Nimbus JwtEncoder/JwtDecoder）+ 接口 RBAC
 * （CUSTOMER/AGENT/ADMIN，role claim → ROLE_*）+ 演示账号 UserDetailsService。
 *
 * <p>登录入口 {@code POST /api/auth/login} 与可观测端点、Knife4j 文档路径 permitAll，
 * 其余 {@code /api/**} 一律要求已认证（威胁登记 T-06-02 Spoofing 缓解）。
 *
 * @author flywhl
 * @since 1.0.0
 */
...
public class SecurityConfig {
    ...
    @Bean
    JwtEncoder jwtEncoder(ScsProperties properties) {
        SecretKey secretKey = jwtSecretKey(properties);
        return new NimbusJwtEncoder(new ImmutableSecret<>(secretKey));
    }

    @Bean
    JwtDecoder jwtDecoder(ScsProperties properties) {
        return NimbusJwtDecoder.withSecretKey(jwtSecretKey(properties)).build();
    }

    private UserDetails toUserDetails(SysUser user) {
        return User.builder()
    ...
    private static SecretKey jwtSecretKey(ScsProperties properties) {
