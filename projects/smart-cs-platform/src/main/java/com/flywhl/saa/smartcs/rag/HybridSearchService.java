...
/**
 * FAQ 混合检索：Milvus 向量检索（{@code milvusVectorStore}）+ Elasticsearch 全文 match（原生
 * {@link RestClient}）+ Reciprocal Rank Fusion 融合，直接移植 {@code 26-es-hybrid-demo} 核心算法。
 *
 * <p>Spring AI 1.1.2 的 {@code ElasticsearchVectorStore} 仅提供向量 similaritySearch；混合检索
 * 在应用层用原生 match 查询补齐全文通道，再做 RRF 融合，无 Spring AI 内置 Hybrid API 依赖。
 *
 * @author flywhl
 * @since 1.0.0
 */
...
public class HybridSearchService {
    ...
    public HybridSearchService(
            ...
            @Value("${spring.ai.vectorstore.elasticsearch.index-name:scs-faq}") String indexName) {
    ...
    public List<Document> fullTextSearch(String query, int topK) {
        ...
        try {
            ...
            throw new BizException(CommonResultCode.INTERNAL_ERROR, "Elasticsearch 全文检索失败: " + ex.getMessage(), ex);
        }
    }
