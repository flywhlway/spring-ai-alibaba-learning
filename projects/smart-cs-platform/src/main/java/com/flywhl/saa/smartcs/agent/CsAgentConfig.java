 ...
 * </ul>
 *
 * <p>本类仅使用 JAR 真源 API；不使用教程简化写法中已废弃/不存在的总控 Agent 类型、
 * 子 Agent 转工具的旧式静态工厂方法、模型执行前中断钩子、以及路由表映射构造方式。
 *
 * @author flywhl
 * @since 1.0.0
 */
...
public class CsAgentConfig {
    ...
    @Bean
    ReactAgent orderAgent(ChatModel dashScopeChatModel, OrderTool orderTool) {
        return ReactAgent.builder()
                .name("order-agent")
    ...
    @Bean
    ReactAgent afterSalesAgent(ChatModel dashScopeChatModel, TicketTools ticketTools) {
        return ReactAgent.builder()
    ...
    @Bean
    ReactAgent techSupportAgent(ChatModel dashScopeChatModel, TicketTools ticketTools) {
        return ReactAgent.builder()
    ...
    @Bean
    ReactAgent ticketAgent(ChatModel dashScopeChatModel, TicketTools ticketTools) {
        return ReactAgent.builder()
                .name("ticket-agent")
